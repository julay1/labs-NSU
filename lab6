...
void calc_height(struct tree *tree) { //считаем высоту дерева
...
struct tree* left_rot(struct tree* tree) { //левый поворот 
...
struct tree* right_rot(struct tree* tree) {// правый поворот 
